//#define UI_SIMULATE_RUNTIME //模拟非编辑器状态  在编辑器使用 非编辑器加载模式 用于在编辑器下测试  
...
namespace Zeng.GameFrame.UIS
{
    ...
    public static class UIBindHelper
    {
        ...
        /// 根据创建时的类获取
        /// </summary>
        private static Dictionary<Type, UIBindVo> g_UITypeToPkgInfo = new ();

        /// <summary>
        /// 根据 pkg + res 双字典获取
        /// </summary>
        private static Dictionary<string, Dictionary<string, UIBindVo>> g_UIPathToPkgInfo = new ();

        /// <summary>
        /// 只有panel 的信息
        /// </summary>
        private static Dictionary<string, UIBindVo> g_UIPanelNameToPkgInfo = new ();

        //改为dll过后 提供给外部的方法
        //1 从UI工具中自动生成绑定代码
        //2 外部请直接调用此方法 UIBindHelper.InternalGameGetUIBindVoFunc = UICodeGenerated.UIBindProvider.Get;
        ...
        public static bool InitAllBind()
        {
            ...
            #if !UNITY_EDITOR || UI_SIMULATE_RUNTIME
